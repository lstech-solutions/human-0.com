...
export async function GET(request: Request) {
  try {
    ...
    if (locale === 'en') {
      ...
      const localizedPath = path.resolve(process.cwd(), `docs/i18n/${locale}/privacy.md`);
      const devLocalizedPath = path.resolve(process.cwd(), `../docs/i18n/${locale}/docusaurus-plugin-content-docs/current/privacy.md`);
